  ...
  <div id="main">
    ...
    <div id="buttons">
      <div id="button-content">
        <el-row>
          <el-button type="success" plain @click="updateForm('ruleForm')"> 点击修改 </el-button>
        </el-row>
      </div>
    </div>
...
import {getBlogById, submitBlog, updateBlog} from "../../api/editBlog";
...
export default {
    ...
    return {
      ...
      rules: {
        title: [
          ...
          {min: 2, max: 100, message: '标题过长!!', trigger: 'blur'}
        ],
        digest: [
  ...
  async created() {
    ...
    if( result.status === 0){
      ...
      this.ruleForm.tag = blog.tag
      this.ruleForm.digest = blog.digest
      this.ruleForm.title = blog.title
      this.ruleForm.classification = blog.classification
      this.ruleForm.field = blog.field
    }
    else{
      alert(result.msg)
    }
