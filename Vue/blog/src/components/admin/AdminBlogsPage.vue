    ...
    <li v-for="item in blogs">
      <div id="block">
        <div id="arrangement">
          ...
          <br>
          <p class="blog-user">Author: {{item.author}}</p>
        ...
        <div id="operation">
            <span>
<!--              <el-button @click="item.show = true">查看详情</el-button>-->
      ...
      <div class="blog-content">
        <div id="markdown" v-show="true">
          <v-md-editor v-model="item.content" mode="preview" height="250px" right-toolbar="toc sync-scroll"></v-md-editor>
        </div>
      </div>
...
import {getBlogPageNum, getLatestBlogs} from "../../api/editBlog";
...
export default defineComponent({
  ...
  async setup(){
    ...
    let blogs = reactive([])
    let response = await getBlogPageNum("", "allTmp")

    pageNum = response.data.pageNum
    response = await getLatestBlogs(1, "allTmp", "")
  ...
  methods: {
    ...
    async remove(item){
      const response = await DeleteBlogs(item)
      if (response.status !== 0){
        alert(response.msg)
      }else{
        alert(response.msg)
    ...
    checkBlog(item){
      window.location = '/admin/checkblogs/' + item.blogId
    },
    async getPageBlogs(val){
      const response = await getLatestBlogs(val, "allTmp", "")
